...
2. Add a PostgreSQL service in the same Railway project as the Django app.
3. In the Django service Variables, confirm `DATABASE_URL` references the PostgreSQL service, for example `${{Postgres.DATABASE_URL}}`. A PostgreSQL service that is not linked to the app will leave the app without a persistent database.
